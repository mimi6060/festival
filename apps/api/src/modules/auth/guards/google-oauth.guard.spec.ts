/**
 * Google OAuth Guard Unit Tests
 *
 * Comprehensive tests for Google OAuth guard including:
...
describe('GoogleOAuthGuard', () => {
  ...
  const setupGuard = async (config: Record<string, string | boolean | undefined>) => {
    ...
    return module.get<GoogleOAuthGuard>(GoogleOAuthGuard);
  };
  ...
  describe('canActivate', () => {
    describe('OAuth disabled', () => {
      it('should throw BadRequestException when GOOGLE_OAUTH_ENABLED is false string', async () => {
        guard = await setupGuard({
      ...
      it('should throw BadRequestException when GOOGLE_OAUTH_ENABLED is boolean false', async () => {
        guard = await setupGuard({
          GOOGLE_OAUTH_ENABLED: false,
          GOOGLE_CLIENT_ID: 'test-client-id',
          GOOGLE_CLIENT_SECRET: 'test-client-secret',
      ...
      it('should throw BadRequestException when GOOGLE_OAUTH_ENABLED is undefined', async () => {
        guard = await setupGuard({
    ...
    describe('missing credentials', () => {
      it('should throw BadRequestException when GOOGLE_CLIENT_ID is missing', async () => {
        guard = await setupGuard({
          GOOGLE_OAUTH_ENABLED: 'true',
      ...
      it('should throw BadRequestException when GOOGLE_CLIENT_SECRET is missing', async () => {
        guard = await setupGuard({
          GOOGLE_OAUTH_ENABLED: 'true',
          GOOGLE_CLIENT_ID: 'test-client-id',
      ...
      it('should throw BadRequestException when GOOGLE_CLIENT_ID is "disabled"', async () => {
        guard = await setupGuard({
          GOOGLE_OAUTH_ENABLED: 'true',
      ...
      it('should throw BadRequestException when GOOGLE_CLIENT_SECRET is "disabled"', async () => {
        guard = await setupGuard({
          GOOGLE_OAUTH_ENABLED: 'true',
          GOOGLE_CLIENT_ID: 'test-client-id',
    ...
    describe('OAuth enabled with valid credentials', () => {
      it('should accept string "true" for GOOGLE_OAUTH_ENABLED', async () => {
        guard = await setupGuard({
          GOOGLE_OAUTH_ENABLED: 'true',
          GOOGLE_CLIENT_ID: 'test-client-id',
          GOOGLE_CLIENT_SECRET: 'test-client-secret',
      ...
      it('should accept boolean true for GOOGLE_OAUTH_ENABLED', async () => {
        guard = await setupGuard({
          GOOGLE_OAUTH_ENABLED: true,
          GOOGLE_CLIENT_ID: 'test-client-id',
          GOOGLE_CLIENT_SECRET: 'test-client-secret',
  ...
  describe('handleRequest', () => {
    ...
    it('should return user when user exists and no error', () => {
      const user = { id: 'user-123', email: 'test@gmail.com' };
    ...
    it('should throw error when user is null', () => {
      expect(() => guard.handleRequest(null, null)).toThrow('Google authentication failed');
    });

    it('should throw error when user is undefined', () => {
      expect(() => guard.handleRequest(null, undefined)).toThrow('Google authentication failed');
    });
