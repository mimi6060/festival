 ...
 *
 * React Native hook for locale-aware currency formatting.
...
/**
 * Hook for locale-aware currency formatting in React Native
 *
 ...
 *   return (
 *     <View>
 *       <Text>Price: {formatPriceOrFree(ticket.price, 'Free entry')}</Text>
 *       <Text>Revenue: {formatCompact(ticket.totalRevenue)}</Text>
 *     </View>
 *   );
 * }
 * ```
...
export function useFormatCurrency(): UseFormatCurrencyReturn {
  ...
  const defaultCurrency = useMemo(() => getDefaultCurrencyFn(language), [language]);

  const formatCurrency = useCallback(
    (amount: number, options?: CurrencyFormatterOptions) =>
      formatCurrencyFn(amount, language, options),
    [language]
  );

  const formatNarrow = useCallback(
    (amount: number, currency?: string) => formatCurrencyNarrowFn(amount, language, currency),
    [language]
  );

  const formatCode = useCallback(
    (amount: number, currency?: string) => formatCurrencyCodeFn(amount, language, currency),
    [language]
  );

  const formatName = useCallback(
    (amount: number, currency?: string) => formatCurrencyNameFn(amount, language, currency),
    [language]
  );

  const formatCompact = useCallback(
    (amount: number, currency?: string) => formatCurrencyCompactFn(amount, language, currency),
    [language]
  );

  const formatRange = useCallback(
    (minAmount: number, maxAmount: number, currency?: string) =>
      formatCurrencyRangeFn(minAmount, maxAmount, language, currency),
    [language]
  );

  const formatWithSymbol = useCallback(
    (amount: number, currency?: string) => formatWithSymbolFn(amount, language, currency),
    [language]
  );

  const formatAccounting = useCallback(
    (amount: number, currency?: string) => formatAccountingCurrencyFn(amount, language, currency),
    [language]
  );

  const formatPrice = useCallback(
    (price: number, currency?: string) => formatPriceFn(price, language, currency),
    [language]
  );

  const formatPriceOrFree = useCallback(
    (price: number, freeText = 'Free', currency?: string) =>
      formatPriceOrFreeFn(price, language, freeText, currency),
    [language]
  );

  const parseCurrency = useCallback(
    (value: string) => parseCurrencyFn(value, language),
    [language]
  );
