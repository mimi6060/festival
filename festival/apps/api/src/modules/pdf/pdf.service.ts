...
export class PdfService {
  ...
  private async createTicketPdf(ticket: any): Promise<Buffer> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        doc.fontSize(28).font('Helvetica-Bold').fillColor('#ffffff').text(ticket.festival.name.toUpperCase(), 40, 35, { width: pageWidth - 80, align: 'center' });

        const hash = crypto.createHmac('sha256', this.qrSecret).update(ticket.id).digest('hex').substring(0, 16).toUpperCase();
        try {
        ...
        doc.fontSize(9).fillColor(this.colors.textLight).text('Hash: ' + hash, { align: 'center' });
        doc.moveDown(2);
        doc.fontSize(11).fillColor(this.colors.text).text('Lieu: ' + ticket.festival.location, 60);
        doc.text('Date: ' + this.formatDate(ticket.festival.startDate) + ' - ' + this.formatDate(ticket.festival.endDate), 60);
  ...
  private async createInvoicePdf(payment: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        const pageWidth = doc.page.width;
        const invoiceNum = 'FAC-' + new Date().getFullYear() + '-' + payment.id.substring(0, 8).toUpperCase();

        doc.rect(0, 0, pageWidth, 100).fill(this.colors.primary);
        doc.fontSize(24).font('Helvetica-Bold').fillColor('#ffffff').text(this.companyInfo.name, 50, 30);
        doc.fontSize(28).text('FACTURE', pageWidth - 200, 30, { width: 150, align: 'right' });
        doc.fontSize(10).font('Helvetica').text(invoiceNum, pageWidth - 200, 60, { width: 150, align: 'right' });
        ...
        doc.font('Helvetica').fillColor(this.colors.text).text(payment.user.firstName + ' ' + payment.user.lastName, 360, 148);
        ...
        doc.fontSize(10).font('Helvetica-Bold').fillColor('#ffffff').text('Description', 60, tableTop + 8);
        ...
        doc.fontSize(10).fillColor(this.colors.text).text('Sous-total HT:', 340, y + 7);
        doc.text(ht.toFixed(2) + ' ' + payment.currency, 440, y + 7, { width: 50, align: 'right' });
        y += 25;
        doc.rect(330, y, 170, 25).fillAndStroke('#f5f5f5', this.colors.border);
        doc.text('TVA (20%):', 340, y + 7);
        doc.text(tva.toFixed(2) + ' ' + payment.currency, 440, y + 7, { width: 50, align: 'right' });
        y += 25;
        doc.rect(330, y, 170, 35).fill(this.colors.primary);
        doc.fontSize(12).font('Helvetica-Bold').fillColor('#ffffff').text('TOTAL TTC:', 340, y + 12);
        doc.text(total.toFixed(2) + ' ' + payment.currency, 420, y + 12, { width: 70, align: 'right' });

        doc.fontSize(8).fillColor('#999999').text('Facture generee automatiquement.', 50, doc.page.height - 50, { align: 'center', width: pageWidth - 100 });
        doc.end();
      } catch (e) { reject(e); }
    });
  }

  async generateBadgePdf(assignmentId: string, userId: string, photoBuffer?: Buffer): Promise<Buffer> {
  ...
  private async createBadgePdf(assignment: any, photoBuffer?: Buffer): Promise<Buffer> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const pageWidth = 340;
        const roleColors: Record<string, string> = { ADMIN: '#dc2626', ORGANIZER: '#7c3aed', STAFF: '#2563eb', CASHIER: '#059669', SECURITY: '#d97706' };
        const roleColor = roleColors[assignment.role] || '#2563eb';
        const accessLevels: Record<string, string> = { ADMIN: 'FULL', ORGANIZER: 'FULL', SECURITY: 'HIGH', CASHIER: 'MEDIUM', STAFF: 'LOW' };
        const accessLevel = accessLevels[assignment.role] || 'LOW';
        const accessColors: Record<string, string> = { LOW: '#6b7280', MEDIUM: '#3b82f6', HIGH: '#f59e0b', FULL: '#dc2626' };

        doc.rect(0, 0, pageWidth, 90).fill(roleColor);
        doc.rect(0, 90, pageWidth, 8).fill(accessColors[accessLevel]);
        doc.fontSize(14).font('Helvetica-Bold').fillColor('#ffffff').text(assignment.festival.name.toUpperCase(), 15, 15, { width: pageWidth - 30, align: 'center' });
        const roleNames: Record<string, string> = { ADMIN: 'ADMINISTRATEUR', ORGANIZER: 'ORGANISATEUR', STAFF: 'STAFF', CASHIER: 'CAISSIER', SECURITY: 'SECURITE' };
        doc.fontSize(20).text(roleNames[assignment.role] || assignment.role, 15, 40, { width: pageWidth - 30, align: 'center' });
        doc.fontSize(9).font('Helvetica').text('STAFF-' + assignment.id.substring(0, 6).toUpperCase(), 15, 70, { width: pageWidth - 30, align: 'center' });

        const photoY = 115, photoSize = 100, photoX = (pageWidth - photoSize) / 2;
        if (photoBuffer) {
          doc.save();
          ...
          doc.restore();
          doc.circle(photoX + photoSize / 2, photoY + photoSize / 2, photoSize / 2 + 2).lineWidth(3).stroke(roleColor);
        } else {
          doc.circle(photoX + photoSize / 2, photoY + photoSize / 2, photoSize / 2).fillAndStroke('#e0e0e0', this.colors.border);
          doc.fontSize(11).fillColor(this.colors.textLight).text('PHOTO', photoX, photoY + photoSize / 2 - 5, { width: photoSize, align: 'center' });
        }

        doc.fontSize(18).font('Helvetica-Bold').fillColor(this.colors.primary).text(assignment.user.firstName.toUpperCase(), 15, photoY + photoSize + 20, { width: pageWidth - 30, align: 'center' });
        doc.fontSize(20).text(assignment.user.lastName.toUpperCase(), 15, doc.y, { width: pageWidth - 30, align: 'center' });

        if (assignment.zone) {
          const infoBoxY = doc.y + 15;
          doc.roundedRect(20, infoBoxY, pageWidth - 40, 40, 5).fillAndStroke('#f5f5f5', this.colors.border);
          doc.fontSize(8).font('Helvetica-Bold').fillColor(this.colors.textLight).text('ZONE ASSIGNEE', 30, infoBoxY + 8);
          doc.fontSize(12).fillColor(this.colors.text).text(assignment.zone.name, 30, infoBoxY + 22);
          doc.y = infoBoxY + 50;
        }

        try {
          const qrData = JSON.stringify({ type: 'STAFF', id: assignment.id, staff: assignment.userId, role: assignment.role, level: accessLevel });
        ...
        doc.fontSize(8).font('Helvetica').fillColor(this.colors.textLight).text('Valide: ' + this.formatDate(assignment.startTime) + ' - ' + this.formatDate(assignment.endTime), 15, doc.y, { width: pageWidth - 30, align: 'center' });
        const levelBadgeX = (pageWidth - 120) / 2;
        doc.roundedRect(levelBadgeX, doc.y + 10, 120, 22, 11).fill(accessColors[accessLevel]);
        doc.fontSize(10).font('Helvetica-Bold').fillColor('#ffffff').text('ACCES ' + accessLevel, levelBadgeX, doc.y + 16, { width: 120, align: 'center' });
        doc.fontSize(6).fillColor('#aaaaaa').text('Badge personnel et non cessible', 15, 515, { width: pageWidth - 30, align: 'center' });
        doc.end();
      } catch (e) { reject(e); }
    });
  }
  ...
  private async createProgramPdf(festival: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.fontSize(48).font('Helvetica-Bold').fillColor('#ffffff').text(festival.name.toUpperCase(), 40, 100, { width: pageWidth - 80, align: 'center' });
        ...
        for (const [day, stages] of performancesByDay) {
          ...
          for (const stageData of stages) {
            ...
            doc.fontSize(14).font('Helvetica-Bold').fillColor('#ffffff').text(stageData.stage.toUpperCase(), 55, doc.y - 25);
            doc.y += 10;
        ...
        doc.fontSize(14).font('Helvetica').fillColor(this.colors.textLight).text('Programme genere le ' + this.formatDateTime(new Date()), 40, 300, { width: pageWidth - 80, align: 'center' });
        doc.moveDown(2);
        doc.fontSize(11).text('Les horaires sont susceptibles de changer.', { align: 'center', width: pageWidth - 80 });
        doc.fontSize(20).font('Helvetica-Bold').fillColor('#ffffff').text(festival.name, 40, doc.page.height - 120, { width: pageWidth - 80, align: 'center' });
        doc.end();
      } catch (e) { reject(e); }
    });
  }
  ...
  async generateFinancialReportPdf(festivalId: string, userId: string): Promise<Buffer> {
    ...
    return this.createFinancialReportPdf({
      ...
      refunds: { count: refundedTickets.length, totalAmount: refundAmount, byReason: [{ reason: 'Annulation', count: refundedTickets.length, amount: refundAmount }] },
    });
  }
  ...
  private async createFinancialReportPdf(data: FinancialReportPdfData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.fontSize(28).font('Helvetica-Bold').fillColor('#ffffff').text('RAPPORT FINANCIER', 40, 40);
        doc.fontSize(20).font('Helvetica').text(data.festivalName, 40, 80);
        doc.fontSize(12).text('Periode: ' + this.formatDate(data.period.startDate) + ' - ' + this.formatDate(data.period.endDate), 40, 115);

        doc.y = 180;
        doc.fontSize(10).font('Helvetica').fillColor(this.colors.textLight).text('Genere le: ' + this.formatDateTime(data.generatedAt));
        doc.text('Par: ' + data.generatedBy);
        doc.moveDown(2);

        const boxWidth = (pageWidth - 100) / 3;
        const summaryY = doc.y;
        this.addMetricBox(doc, 'Chiffre d\'Affaires', data.summary.totalRevenue.toFixed(2) + ' EUR', 40, summaryY, boxWidth, this.colors.success);
        this.addMetricBox(doc, 'Benefice Net', data.summary.netProfit.toFixed(2) + ' EUR', 40 + boxWidth + 10, summaryY, boxWidth, this.colors.primary);
        this.addMetricBox(doc, 'Marge', data.summary.profitMargin.toFixed(1) + '%', 40 + (boxWidth + 10) * 2, summaryY, boxWidth, this.colors.accent);
        doc.y = summaryY + 90;

        doc.rect(40, doc.y, pageWidth - 80, 30).fill(this.colors.accent);
        doc.fontSize(12).font('Helvetica-Bold').fillColor('#ffffff').text('REPARTITION REVENUS', 55, doc.y - 22);
        doc.moveDown();

        doc.fontSize(12).font('Helvetica-Bold').fillColor(this.colors.text).text('Billetterie: ' + data.revenueBreakdown.ticketSales.total.toFixed(2) + ' EUR', 50);
        doc.fontSize(10).font('Helvetica').fillColor(this.colors.textLight).text('Cashless: Recharges ' + data.revenueBreakdown.cashless.totalTopups.toFixed(2) + ' EUR | Paiements ' + data.revenueBreakdown.cashless.totalPayments.toFixed(2) + ' EUR', 50);
        doc.text('Vendors: Ventes ' + data.revenueBreakdown.vendors.totalSales.toFixed(2) + ' EUR | Commission ' + data.revenueBreakdown.vendors.commission.toFixed(2) + ' EUR', 50);
        doc.text('Camping: ' + data.revenueBreakdown.camping.revenue.toFixed(2) + ' EUR', 50);
        doc.moveDown(2);

        doc.rect(40, doc.y, pageWidth - 80, 30).fill('#059669');
        doc.fontSize(12).font('Helvetica-Bold').fillColor('#ffffff').text('RESUME TVA', 55, doc.y - 22);
        doc.moveDown();
        doc.fontSize(10).font('Helvetica').fillColor(this.colors.text).text('Total TVA collectee: ' + data.taxSummary.totalTaxCollected.toFixed(2) + ' EUR', 50);
        doc.moveDown(2);

        doc.rect(40, doc.y, pageWidth - 80, 30).fill('#dc2626');
        doc.fontSize(12).font('Helvetica-Bold').fillColor('#ffffff').text('REMBOURSEMENTS', 55, doc.y - 22);
        doc.moveDown();
        doc.fontSize(10).font('Helvetica').fillColor(this.colors.text).text(data.refunds.count + ' remboursements pour ' + data.refunds.totalAmount.toFixed(2) + ' EUR', 50);

        doc.fontSize(8).fillColor('#999999').text('Rapport genere automatiquement.', 40, doc.page.height - 50, { align: 'center', width: pageWidth - 80 });
        doc.end();
      } catch (e) { reject(e); }
    });
  }
  ...
  private async createReceiptPdf(payment: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.fontSize(14).font('Helvetica-Bold').fillColor(this.colors.primary).text(this.companyInfo.name, { align: 'center' });
        doc.moveDown(0.5);
        doc.fontSize(12).text('RECU DE PAIEMENT', { align: 'center' });
        doc.moveDown();
        doc.fontSize(9).font('Helvetica').fillColor(this.colors.text).text('N: REC-' + payment.id.substring(0, 8).toUpperCase(), { align: 'center' });
        doc.text('Date: ' + this.formatDateTime(payment.paidAt || payment.createdAt), { align: 'center' });
        doc.moveDown();

        for (const t of payment.tickets) {
          doc.text(t.festival.name + ' - ' + t.category.name + ': ' + Number(t.purchasePrice).toFixed(2) + ' ' + payment.currency);
        }
        doc.moveDown();
        doc.fontSize(12).font('Helvetica-Bold').text('TOTAL: ' + Number(payment.amount).toFixed(2) + ' ' + payment.currency, { align: 'center' });
        doc.moveDown(2);
  ...
  private async createCampingVoucherPdf(booking: any): Promise<Buffer> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        doc.fontSize(24).font('Helvetica-Bold').fillColor(this.colors.primary).text('BON DE CAMPING', { align: 'center' });
        doc.moveDown(0.5);
        doc.fontSize(18).font('Helvetica').fillColor(this.colors.text).text(booking.spot.zone.festival.name, { align: 'center' });
        doc.fontSize(12).fillColor(this.colors.textLight).text('CAMP-' + booking.id.substring(0, 8).toUpperCase(), { align: 'center' });
        doc.moveDown(2);
        ...
        doc.font('Helvetica').text(booking.user.firstName + ' ' + booking.user.lastName, 50).text(booking.user.email, 50);
        doc.moveDown();
        doc.font('Helvetica-Bold').text('DATES', 50);
        doc.font('Helvetica').text('Arrivee: ' + this.formatDateTime(booking.checkIn), 50).text('Depart: ' + this.formatDateTime(booking.checkOut), 50);
        doc.moveDown();
        doc.font('Helvetica-Bold').text('EMPLACEMENT', 50);
        doc.font('Helvetica').text('Type: ' + booking.spot.zone.type, 50).text('Nom: ' + booking.spot.zone.name + ' - ' + booking.spot.number, 50);
        doc.moveDown(2);
        doc.fontSize(16).font('Helvetica-Bold').fillColor(this.colors.primary).text('TOTAL: ' + Number(booking.totalPrice).toFixed(2) + ' EUR', { align: 'center' });
        doc.end();
      } catch (e) { reject(e); }
    });
  }
  ...
  private async createRefundConfirmationPdf(payment: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.fontSize(18).font('Helvetica-Bold').fillColor(this.colors.primary).text(this.companyInfo.name, 50, 50);
        doc.fontSize(24).text('CONFIRMATION DE REMBOURSEMENT', 50, 100);
        doc.fontSize(12).font('Helvetica').fillColor(this.colors.text).text('Reference: RMB-' + payment.id.substring(0, 8).toUpperCase(), 50, 130);
        doc.text('Date: ' + this.formatDateTime(payment.refundedAt || new Date()), 50, 145);
        doc.moveDown(2);

        doc.rect(50, doc.y, 150, 30).fill(this.colors.success);
        doc.fontSize(12).font('Helvetica-Bold').fillColor('#ffffff').text('REMBOURSE', 55, doc.y - 22);
        doc.moveDown(2);

        doc.fontSize(10).font('Helvetica-Bold').fillColor(this.colors.text).text('BENEFICIAIRE:', 50);
        doc.font('Helvetica').fillColor(this.colors.textLight).text(payment.user.firstName + ' ' + payment.user.lastName).text(payment.user.email);
        doc.moveDown();
        ...
        doc.moveDown(2);
        doc.fontSize(14).font('Helvetica-Bold').fillColor(this.colors.primary).text('TOTAL REMBOURSE: ' + Number(payment.amount).toFixed(2) + ' ' + payment.currency, { align: 'center' });
        doc.end();
      } catch (e) { reject(e); }
    });
  }
